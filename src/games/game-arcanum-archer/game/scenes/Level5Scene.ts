...
export class Level5 extends Phaser.Scene {
  ...
  private currentLevelKey: string = 'level5';
  ...
  private levelMaxArrows: number = 4; // <-- SE DEFINE CUÁNTOS DISPAROS TIENE EL JUGADOR
  ...
  //Dialogos
  private dialogsData = globalState?.dialogs?.level5?.question?.text || [
    `¿Te atreves a aprender lo que otros prefieren ignorar?
Me llaman Draco. Algunos me evitan, otros me buscan… pero todos recuerdan lo que enseño.
Solo una pregunta te haré. Una verdad velada en sombras.
Respóndela bien, y te obsequiaré mi Poción de la Paciencia. ☠️ +1`
  ];
...
`
    ...
    if (!this.anims.exists('raven-fly')) {
      this.anims.create({
        key: 'raven-fly',
        frames: this.anims.generateFrameNumbers('ravenTarget', {
          start: 0,
          end: 5
        }),
        frameRate: 5,
    ...
    // Encuentra la pregunta del nivel 5 ANTES de crear los targets
    const level5QuestionData = globalState.questions.find((q) => q.id === 'level-5');
    // Valida que la pregunta exista y tenga opciones
    if (!level5QuestionData || !level5QuestionData.options || level5QuestionData.options.length < 4) {
      console.error('¡Datos de la pregunta del Nivel 5 no encontrados o incompletos!');
    ...
    targetsPositions.forEach((pos, index) => {
      ...
      let startX: number;
      let initialVelocityX: number;
      let initialFlipX: boolean;

      // --- Decidir dirección y velocidad inicial ---
      if (index % 2 === 0) {
        // Empieza Izquierda -> Derecha
        startX = -50;
        initialVelocityX = randomSpeed;
        initialFlipX = false;
      } else {
        // Empieza Derecha -> Izquierda
        startX = this.scale.width + 50;
        initialVelocityX = -randomSpeed;
        initialFlipX = true;
      }

      const startY = Phaser.Math.Between(150, 450); // Altura inicial aleatoria

      // --- Crear el sprite con física ---
      const targetSprite = this.physics.add
        .sprite(startX, startY, pos.texture) // Posición inicial
        .setScale(pos.size)
        .setImmovable(true)
        .setName(pos.id);

      // Configuración de física básica
      targetSprite.body?.setAllowGravity(false);
      targetSprite.setCollideWorldBounds(false); // ¡Importante! No queremos que choque con los bordes

      // --- Establecer velocidad y flip inicial ---
      targetSprite.setVelocityX(initialVelocityX);
      targetSprite.setFlipX(initialFlipX); // Orientación inicial basada en la velocidad
      ...
      // Animación
      if (pos.texture === 'ravenTarget') {
        targetSprite.play('raven-fly');
      } else {
        console.warn(`Textura desconocida: ${pos.texture}`);
      }

      // --- CREACIÓN DEL BOTÓN DOM Y DATOS ASOCIADOS ---
      const optionLetter = level5QuestionData.optionsIndex[pos.optionIndex];
      const optionText = level5QuestionData.options[pos.optionIndex]; // Necesitas el texto completo para aria-label
      // Calcular el offset Y para el botón DOM (puede ser el mismo o ajustado)
      const buttonYOffset = -targetSprite.displayHeight / 2 - 25; // Ajusta este valor si es necesario

      const labelButtonDOM = this.add
        .dom(targetSprite.x, targetSprite.y + buttonYOffset)
        .createFromHTML(
          `
    <button class="target-label-button"
  ...
  `
      ...
      // Guardar referencias importantes en el sprite
      targetSprite.setData('labelButton', labelButtonDOM);
      targetSprite.setData('buttonYOffset', buttonYOffset); // Guardar el offset Y para update
      targetSprite.setData('optionLetter', optionLetter); // Para anuncios
      targetSprite.setData('optionText', optionText); // Para anuncios/tooltips
      targetSprite.setData('isCorrect', pos.optionIndex === level5QuestionData.correctIndex); // Para handleTargetHit
      targetSprite.setData('loopIndex', index); // Para accesibilidad y refocus

      // --- Configurar Interacción del Botón DOM (Listeners) ---
      ...
      if (buttonElement) {
        buttonElement.addEventListener('click', () => {
          ...
          if (buttonIndex !== -1) {
            this.focusedTargetIndex = buttonIndex; // Actualiza el índice de foco lógico
            this.updateFocusVisual(); // Si tienes un indicador visual Phaser
            // Podrías incluso simular un disparo con teclado aquí si quisieras
          }
        });
  ...
  private addQuestionToDialogs(): void {
    if (globalState.questions.length > 0) {
      const question = globalState.questions.find((q) => q.id === 'level-5');
      if (question) {
  ...
  private handleTargetHit(arrowObj: Phaser.Physics.Arcade.Image, targetObj: Phaser.Physics.Arcade.Image): void {
    ...
    const level5Question = globalState.questions.find((q) => q.id === 'level-5');
    // OBTENER EL BOTÓN DOM
    ...
    if (
      ...
      console.error(`Datos incompletos en handleTargetHit para target ${targetId} (Level 5)`);
      if (labelButtonDOM) labelButtonDOM.destroy(); // Destruir botón si existe
      if (target.active) {
    ...
    // DESTRUIR EL BOTÓN DOM ASOCIADO AL TARGET GOLPEADO
    if (labelButtonDOM) {
    ...
    if (isCorrect) {
      ...
      if (target.active) {
        // Destruir el target correcto inmediatamente
        target.setActive(false).setVisible(false);
        if (target.body) target.body.enable = false;
        target.destroy();
      }
      this.showRewardFeedback('potion_icon', target.x, target.y + 80); // Ajusta Y si es necesario
      const rewardData = level5Question.reward;
      let rewardText = '';
      if (rewardData) {
      ...
      this.time.delayedCall(3000, () => {
        ...
        globalState.targets = [];
        if (this.keyboardFocusIndicator) this.keyboardFocusIndicator.clear();
        this.scene.start('mapScene');
      });
    } else {
      // --- INCORRECTO ---
      this.tweens.killTweensOf(target); // Detener movimiento horizontal
      this.cameras.main.shake(200, 0.01);
      target.setTint(0xff0000);
      target.setActive(false); // Para lógica de refocus y evitar más colisiones
      target.setVisible(true); // Para ver la caída

      if (target.body) {
        const body = target.body as Phaser.Physics.Arcade.Body;
        body.enable = true;
        body.setAllowGravity(true);
        body.setCollideWorldBounds(false); // PERMITIR QUE CAIGA
        body.setVelocity(Phaser.Math.Between(-60, 60), Phaser.Math.Between(100, 200)); // Empujón
        body.setAngularVelocity(Phaser.Math.Between(-180, 180)); // Giro
        // NO SE DESTRUYE AQUÍ
      } else {
        console.warn(`Target ${targetId} no tiene body físico. Usando Tween para caída.`);
        this.tweens.add({
          targets: target,
          y: this.cameras.main.scrollY + this.cameras.main.height + target.displayHeight,
          alpha: 0.3,
          angle: target.angle + Phaser.Math.Between(-120, 120),
          duration: 2200,
          ease: 'Sine.easeIn',
          onComplete: () => {
  ...
  update(_time: number, delta: number) {
    ...
    // --- Actualizar posición de los targets ---
    const screenWidth = this.scale.width;
    const padding = 50;

    globalState.targets.forEach((targetInfo) => {
      // <--- TU BUCLE EXISTENTE
      const targetSprite = targetInfo.sprite;
      ...
      const velocityX = targetSprite.body.velocity.x;

      if (velocityX < 0 && targetSprite.x < -padding) {
        targetSprite.x = screenWidth + padding;
        targetSprite.y = Phaser.Math.Between(150, 250);
      } else if (velocityX > 0 && targetSprite.x > screenWidth + padding) {
        targetSprite.x = -padding;
        targetSprite.y = Phaser.Math.Between(150, 180);
      }

      // --- NUEVO: ACTUALIZAR POSICIÓN DEL BOTÓN DOM AQUÍ DENTRO DEL BUCLE ---
      const labelButtonDOM = targetSprite.getData('labelButton') as Phaser.GameObjects.DOMElement | null;
      const buttonYOffset = targetSprite.getData('buttonYOffset') as number; // Recuperar el offset

      if (labelButtonDOM && labelButtonDOM.node && labelButtonDOM.active) {
        if (typeof buttonYOffset === 'number') {
          // Asegurarse que buttonYOffset es un número
          labelButtonDOM.setPosition(targetSprite.x, targetSprite.y + buttonYOffset);
        } else {
          // Fallback si buttonYOffset no está, colocarlo justo encima
          labelButtonDOM.setPosition(targetSprite.x, targetSprite.y - targetSprite.displayHeight / 2 - 25);
        }
      }
    });

    // --- Limpieza de Targets Caídos (Incorrectos) ---
    globalState.targets.forEach((targetInfo) => {
      // Ya lo tienes
      const targetSprite = targetInfo.sprite;
      if (targetSprite && !targetSprite.active && targetSprite.visible && targetSprite.body?.allowGravity) {
        const lowerBound = this.cameras.main.scrollY + this.cameras.main.height;
        if (targetSprite.y > lowerBound + targetSprite.displayHeight / 2) {
          targetSprite.destroy(); // Destruir el sprite
          // Considera remover de globalState.targets aquí o dejarlo para shutdown/win
        }
      }
    });
  }
