...
export class Level1 extends Phaser.Scene {
  ...
  private levelMaxArrows: number = 10; // <-- SE DEFINE CUÁNTOS DISPAROS TIENE EL JUGADOR
  ...
  create() {
    ...
    this.dialogHTML = `
      ...
      <img src="assets/game-arcanum-archer/images/Characters/cDvEQj.png" alt="Ilustración de la Profesora Dhevon." class="game-arcanum-book-image1">
    ...
    this.openButton = this.add
      ...
      .createFromHTML(
        `
    <button id="open-dialog-button" class="game-arcanum-btn-open hidden" aria-label="Abrir diálogo de la Profesora Dhevon">
        Abrir Diálogo
    </button>
`
      )
    ...
    if (!level1QuestionData || !level1QuestionData.options || level1QuestionData.options.length < 4) {
      console.error('¡Datos de la pregunta del Nivel 1 no encontrados o incompletos!');
    ...
    targetsPositions.forEach((pos, loopIndex) => {
      ...
      targetSprite.setData('optionText', level1QuestionData.options[pos.optionIndex]); // Texto completo opción
  ...
  private createDialog(): void {
    ...
    // --- CAMBIO AQUÍ: Añadir 'as HTMLElement | null' ---
    ...
    // por lo que addEventListener con 'keydown' y KeyboardEvent es válido.
    dialogContentElement.addEventListener('keydown', (e: KeyboardEvent) => { // <--- ESTA LÍNEA AHORA ES VÁLIDA
      if (e.key === 'Tab') {
...
this.sound.play('getPotion', { volume: 0.01 }); // Sonido de acierto
      ...
      // --- Seleccionar Mensaje de Feedback Correcto ÚNICO ---
          ...
          if (onResultCallback && level1Question) {
            const correctAnswerLetter = level1Question.optionsIndex[level1Question.correctIndex];
            
            onResultCallback({
              isCorrect: true,
              questionIndex: 0, // Level 1 es el índice 0
              selectedAnswer: optionLetter,
              correctAnswer: correctAnswerLetter,
              question: level1Question.question
            });
          }

          // --- EMITIR EVENTO PARA REACT (mantener para compatibilidad) ---
...
private refocusAfterTargetHit(hitLoopIndex: number): void { // <-- Acepta número

  // Filtrar para obtener solo los targets que AÚN están activos
  const currentlyActiveTargetsInfo = globalState.targets
      .map((t, index) => ({ // Mapear para incluir el índice original 0-3
  ...
  if (this.focusedTargetIndex !== -1) {
       ...
       if (focusedTargetData) { // Verificar que exista y que el índice sea válido
 ...
 private focusTargetButtonByIndex(logicalIndex: number): void {
  ...
  private shootArrowKeyboard() {
    ...
    if (currentFocusedLogicalIndex < 0 || currentFocusedLogicalIndex >= globalState.targets.length || !globalState.targets[currentFocusedLogicalIndex]?.sprite?.active) {
        console.error(`>>> ERROR: Índice de foco (${currentFocusedLogicalIndex}) inválido o target en ese índice no está activo! Active count: ${activeTargetCount}`);
