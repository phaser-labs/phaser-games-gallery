...
export class PoliceScene extends Phaser.Scene {
  constructor() {
    super('PoliceScene');
  }

  preload() {
    // this.load.image('policeStation', 'assets/images/policeStation.jpg');
  }
  create() {
    ...
    // create
    const title = this.add.text(600, -100, 'Police Station', {
      fontFamily: 'pixel',
      fontSize: '48px',
      color: '#ffffff',
      stroke: '#000000',
    ...
    const video = this.add.video(400, 270, 'policeStation');

    video.setDisplaySize(93, 90); // Ocupar todo el canvas
    video.setOrigin(0.5);
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'slider/slider.css'; // ajusta según tu estructura pública
    document.head.appendChild(link);

    // 3. Luego de insertar, configurar el slider JS con los datos
    const sliderData = [
      {
        title: 'Police officer	',
        image: 'https://i1.sndcdn.com/artworks-000165384395-rhrjdn-t500x500.jpg',
        audio: 'assets/audio/audio1.mp3',
        description: 'The police officer asked for my ID.'
      },
      {
        title: 'Handcuffs',
