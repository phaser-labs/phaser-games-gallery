  ...
  return `
    ...
    <div class="flex flex-col items-center justify-center ${css.containerQuestion}">
      <div class="mb-4 px-3 py-1 rounded-lg tracking-widest uppercase ${css.indexQuestion}">
        <p data-counter>Question <strong>${index + 1}</strong> of <strong>${total}</strong></p>
      </div>
    ...
    <div class="mt-8 grid grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-3xl ${css.containerQuestion}">
      ${quiz.opciones
        ...
        <div style="pointer-events:auto"
             ...
             class="flex items-center gap-3 p-3 rounded-lg border border-primary/30 dark:bg-slate-800 cursor-pointer hover:border-primary/60 transition-colors">
          <div class="size-8 rounded-full flex items-center justify-center font-bold ${css.indexQuestion}">
            ${OPTION_LETTERS[i]}
          </div>
          <span class="text-sm font-medium text-slate-800 dark:text-white flex-1" data-option>
            ${opcion.texto}
          </span>
        </div>`
        )
