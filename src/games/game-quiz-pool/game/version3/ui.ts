...
import cssV3 from "./QuizPool_version3.module.css"
...
export function createHeaderUI(
  ...
  (el.node as HTMLElement).innerHTML = buildHeaderHTML(quiz,);
  return el;
}
...
function buildHeaderHTML(quiz: Quiz ): string {
  return `
      ...
      <div class="flex items-center gap-6">
        <div class="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700">
          ...
          <div class="flex flex-col leading-none">
            <span class="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Puntos</span>
        ...
        <div class="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700">
          ...
          <div class="flex flex-col leading-none">
            <span class="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Tiempo</span>
...
function buildOptionsHTML(quiz: Quiz): string {
  return `
    <div class="mt-8 grid grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-3xl ${css.containerQuestion}">
      ${quiz.opciones
        .map(
          (opcion, i) => `
        <div style="pointer-events:auto"
             data-option-id="${opcion.id}"
             class="${cssV3.option} flex items-center direction-col gap-3 p-3 rounded-lg border border-primary/30 dark:bg-slate-800 cursor-pointer hover:border-primary/60 transition-colors">
...
export function createPocketLabel(
  ...
  console.log(' Se ejecuto la funcion createPocketLabel');
...
export function createAllPocketLabels(scene: Phaser.Scene, totalOpciones: number): Phaser.GameObjects.DOMElement[] {
  const allLabels = [
    { x: 300, y: 210, letter: 'A' },
    { x: 958, y: 210, letter: 'B' },
    { x: 300, y: 770, letter: 'C' },
    { x: 958, y: 770, letter: 'D' }
  ];
