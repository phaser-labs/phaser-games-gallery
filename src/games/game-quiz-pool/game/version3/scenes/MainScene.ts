...
export class MainScene_v3 extends Phaser.Scene {
  ...
  create() {
    ...
    this.headerEl = createHeaderUI(this, currentQuiz);
    ...
    const map = this.make.tilemap({ key: 'mesaBillar_v3' });
    const tileset = map.addTilesetImage('mesa3', 'mesa_v3');
    if (!tileset) return;
    const mapScale = 1.5;
    const offsetX = (this.scale.width  - map.widthInPixels  * mapScale) / 2;
    const offsetY = (this.scale.height - map.heightInPixels * mapScale) / 2;
    const mesa = map.createLayer('Capa de patrones 1', tileset, offsetX, offsetY);
    mesa?.setScale(mapScale);
    ...
    exitBtn?.addEventListener('click', () => {
      ...
      this.scene.start('MenuScene_v3'); // ← el string debe coincidir con super() de tu MenuScene
    });
    ...
    if (DEBUG_BOUNDS) {
      const g = this.add.graphics();
       ...
       // Dibujar todos los endpoints como puntos rojos
       g.fillStyle(0xff0000, 1);
       for (const seg of ALL_SEGMENTS) {
         g.fillCircle(seg.x1, seg.y1, 3); // punto inicio
         g.fillCircle(seg.x2, seg.y2, 3); // punto fin
       }
  ...
  private _resetWhiteBall() {
    ...
    const waitForBall1 = this.time.addEvent({
      ...
      callback: () => {
        ...
        this.ballWhite.setScale(0.25).setAlpha(1).setVisible(true).setActive(true);
  ...
  private _resetBall1() {
    ...
    this.ball1.setScale(0.25).setAlpha(1).setVisible(true).setActive(true);
