...
import PhaserGame from './versiones/version2/main'; // Importa el juego

// import { globalState } from './core/utils/GlobalState'; // Importa el estado global
import css from './QuizPool.module.css';
export const QuizPool_v2 = ({ dataGame }: { dataGame: Quiz[] }) => {
  // Referencia al contenedor div para el juego
  const gameContainer_v2 = useRef<HTMLDivElement>(null);
  useEffect(() => {
    // Verifica si el contenedor del juego está disponible
    if (!gameContainer_v2.current) return;

    // Crea una nueva instancia del juego de Phaser
    const game = new PhaserGame(gameContainer_v2.current, dataGame);
  ...
  return (
    <>
      ...
      <div
        ...
        <div ref={gameContainer_v2} className={`game-container ${css.quizPool__gameContainer_v2}`} />
      </div>
    </>
  );
};
